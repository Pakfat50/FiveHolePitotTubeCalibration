...
シミュレーション画面は、当該シミュレーションで使用する`CalibrationPlan`に含まれる全較正点を、AoAを縦軸、AoSを横軸とした2次元較正点マップとして表示しなければならない。

#### REQ-SIM-006（現在較正点の強調表示）

シミュレーション実行中、ソフトウェアは現在表示している横面図および正面図と同一の`PointEvaluation`に対応する較正点を、較正点マップ上でその他の較正点と異なる色により強調表示しなければならない。
