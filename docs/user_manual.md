...
本アプリケーションは、5孔ピトー管の較正条件を入力し、較正点マップを確認し、シミュレーションで走査順を確認したうえで、GRBL対応のGコードを生成するためのツールです。

## 2. 最初に行う操作

### 2.1 起動からGコード生成まで

1. python main.pyを実行し、GUIを起動します。
...
以下は、実際の製品GUIを起動してこの一連の操作を行った画面から生成した約30秒の操作GIFです。

![起動からGコード生成まで](media/getting-started.gif)

### 2.2 起動直後

![起動画面](media/01-launch.png)

起動直後は、入力値を設定してください。初期値が表示されていても、実際の装置仕様に合わせて全項目を確認してください。

## 3. 較正条件を設定する

![有効な入力状態](media/02-input-valid.png)
...
![シミュレーション画面](media/03-simulation-start.png)

「シミュレーション」を押すと、較正計画を変更せずに表示を再生します。シミュレーション画面では横面図、正面図、較正点マップ、シークバー、再生／一時停止を確認できます。

![シミュレーション途中](media/04-simulation-progress.png)
...
![Gコード生成完了](media/05-gcode-generated.png)
...
- [製品コードAPI](../api/)
- [トレーサビリティ運用ルール](../traceability-rules/)