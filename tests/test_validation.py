...
@brief InputValidator の入力境界・異常値・複合エラーを検証する。
@details docs/test_specification.md に定義された TEST-UNIT-001..016,111..116 を実装する。
"""
...
class TestInputValidator(unittest.TestCase):
    """@brief InputValidator の要求適合性を確認するテストクラス。"""

    def setUp(self):
        """@brief 各テストで独立した InputValidator を生成する。"""
        self.validator = InputValidator()

    # TEST-UNIT-001
    # Requirements: REQ-VALID-001, REQ-VALID-002
    def test_valid_settings(self):
        """@brief 正常設定が有効として受理されることを確認する。

        @test TEST-UNIT-001: 全入力が仕様範囲内の場合、検証結果が有効でエラーを含まないこと。
        @details 標準の正常設定を validate() に入力し、is_valid と ERROR issue の有無を観測する。
        @par 検証根拠
    ...
    def test_aoa_min_equal_max(self):
        """@brief AoA最小値と最大値が等しい場合を拒否することを確認する。

        @test TEST-UNIT-002: aoa_min == aoa_max は不正入力であること。
        @details 境界条件として最小値と最大値を同値にし、is_valid=False を確認する。
        @par 検証根拠
        仕様が禁止する等号境界を直接入力して検証結果を観測するため、AoA範囲の厳密な大小関係を確認できる。
        @see REQ-INPUT-001, REQ-VALID-002
        """
    ...
    def test_aoa_min_greater_than_max(self):
        """@brief AoA最小値が最大値を超える場合を拒否することを確認する。

        @test TEST-UNIT-003: aoa_min > aoa_max は不正入力であること。
        @details 逆転したAoA範囲を与え、検証結果が無効になることを確認する。
        @par 検証根拠
    ...
    def test_aos_min_equal_max(self):
        """@brief AoS最小値と最大値が等しい場合を拒否することを確認する。

        @test TEST-UNIT-004: aos_min == aos_max は不正入力であること。
        @details AoS範囲の等号境界を入力し、is_valid=False を確認する。
        @par 検証根拠
    ...
    def test_aos_min_greater_than_max(self):
        """@brief AoS最小値が最大値を超える場合を拒否することを確認する。

        @test TEST-UNIT-005: aos_min > aos_max は不正入力であること。
        @details 逆転したAoS範囲を入力し、無効判定を確認する。
        @par 検証根拠
    ...
    def test_aoa_points_minimum_valid(self):
        """@brief AoA点数の下限2点が有効であることを確認する。

        @test TEST-UNIT-006: aoa_points=2 は有効であること。
        @details 許容下限値を入力し、検証が成功することを確認する。
        @par 検証根拠
        下限値そのものを試験することで、境界を1点ずらして誤判定する実装を検出できる。
        @see REQ-INPUT-002, REQ-VALID-002
        """
    ...
    def test_aoa_points_too_small(self):
        """@brief AoA点数が2未満の場合を拒否することを確認する。

        @test TEST-UNIT-007: aoa_points=1 は不正であること。
        @details 下限直下の値を入力し、検証が失敗することを確認する。
        @par 検証根拠
    ...
    def test_aos_points_too_small(self):
        """@brief AoS点数が2未満の場合を拒否することを確認する。

        @test TEST-UNIT-008: aos_points=1 は不正であること。
        @details AoS点数を下限直下に設定し、無効判定を確認する。
        @par 検証根拠
    ...
    def test_feed_rate_invalid(self):
        """@brief Feed rate の0・負値・非有限値を拒否することを確認する。

        @test TEST-UNIT-009: 仕様外のFeed rateはすべて無効であること。
        @details 0、負値、NaN、+InfをsubTestで個別に入力して無効判定を確認する。
        @par 検証根拠
        数値範囲違反と非有限値の両系統を網羅するため、下限制約と有限値制約を同時に確認できる。
        @see REQ-INPUT-004, REQ-VALID-002
        """
    ...
    def test_hold_time_invalid(self):
        """@brief 保持時間の下限未満・非有限値を拒否することを確認する。

        @test TEST-UNIT-010: 0.1秒未満、NaN、+Infの保持時間は無効であること。
        @details 代表的な下限違反値と非有限値を個別に検証する。
        @par 検証根拠
    ...
    def test_distance_non_finite(self):
        """@brief Lx/Lyの非有限値を拒否することを確認する。

        @test TEST-UNIT-011: tip_offset_x/y にNaNまたはInfを指定した場合は無効であること。
        @details X/Y両寸法についてNaNとInfを組み合わせて検証する。
        @par 検証根拠
        両フィールドそれぞれで非有限値を直接入力するため、有限実数であるという仕様を確認できる。
        @see REQ-INPUT-003, REQ-VALID-002
        """
    ...
    def test_x_range_equal(self):
        """@brief X軸可動範囲の最小値と最大値が等しい場合を拒否する。

        @test TEST-UNIT-012: X minimum == maximum は無効であること。
        @details X軸だけを等号境界にして検証結果を確認する。
        @par 検証根拠
        他軸を正常に保ったままX軸だけを違反させるため、X範囲検証を分離して確認できる。
        @see REQ-INPUT-005, REQ-VALID-002
        """
    ...
    def test_y_range_reversed(self):
        """@brief Y軸可動範囲が逆転した場合を拒否する。

        @test TEST-UNIT-013: Y minimum > maximum は無効であること。
        @details Y軸だけを逆転させ、検証結果が無効となることを確認する。
        @par 検証根拠
    ...
    def test_z_range_invalid(self):
        """@brief Z軸可動範囲が成立しない場合を拒否する。

        @test TEST-UNIT-014: Z minimum >= maximum は無効であること。
        @details Z軸の最小・最大を同値にして検証する。
        @par 検証根拠
    ...
    def test_a_range_invalid(self):
        """@brief A軸可動範囲が逆転した場合を拒否する。

        @test TEST-UNIT-015: A minimum > maximum は無効であること。
        @details A軸だけを逆転させ、検証結果を確認する。
        @par 検証根拠
    ...
    def test_multiple_errors_are_reported(self):
        """@brief 複数の入力不正を同時に報告できることを確認する。

        @test TEST-UNIT-016: 複数フィールドが不正な場合、単一エラーで打ち切らず複数issueを返すこと。
        @details AoA範囲、点数、Feed rateを同時に不正化し、ERROR数が3件以上であることを確認する。
        @par 検証根拠
    ...
    def test_lx_zero_is_invalid(self):
        """@brief Lx=0を拒否することを確認する。

        @test TEST-UNIT-111: Lxは0より大きくなければならない。
        @details 仕様境界値0.0を入力して無効判定を確認する。
        @par 検証根拠
    ...
    def test_ly_negative_is_invalid(self):
        """@brief Lyが負値の場合を拒否することを確認する。

        @test TEST-UNIT-112: Ly<=0 は無効であること。
        @details 代表的な負値-0.1を入力して検証結果を確認する。
        @par 検証根拠
    ...
    def test_hold_time_minimum_is_valid(self):
        """@brief 保持時間0.1秒が有効であることを確認する。

        @test TEST-UNIT-113: hold_time_s=0.1 は許容下限として有効であること。
        @details 下限値そのものを入力し、is_valid=Trueを確認する。
        @par 検証根拠
    ...
    def test_hold_time_below_minimum_is_invalid(self):
        """@brief 保持時間が0.1秒をわずかに下回る場合を拒否する。

        @test TEST-UNIT-114: hold_time_s<0.1 は無効であること。
        @details 境界直下0.099999を入力し、無効判定を確認する。
        @par 検証根拠
    ...
    def test_feed_rate_minimum_is_valid(self):
        """@brief Feed rateの下限1.0が有効であることを確認する。

        @test TEST-UNIT-115: feed_rate=1.0 は有効であること。
        @details 許容下限値を入力して検証成功を確認する。
        @par 検証根拠
    ...
    def test_feed_rate_below_minimum_is_invalid(self):
        """@brief Feed rateが1.0をわずかに下回る場合を拒否する。

        @test TEST-UNIT-116: feed_rate<1.0 は無効であること。
        @details 境界直下0.999999を入力して無効判定を確認する。
        @par 検証根拠
