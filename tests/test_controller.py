...
class TestCalibrationController(unittest.TestCase):
    ...
    def test_valid_input_rebuilds_plan(self):
        ...
        TEST ID: TEST-UNIT-084: valid設定変更でvalidate→build_planが実行され、生成planがcurrent_planになること。
        検証根拠:
    ...
    def test_invalid_input_does_not_rebuild_plan(self):
        ...
        TEST ID: TEST-UNIT-085: validateが無効ならbuild_planを呼ばずcan_generate=Falseとすること。
        検証根拠:
    ...
    def test_invalid_then_valid_recovers(self):
        ...
        TEST ID: TEST-UNIT-086: 連続2回の設定変更でinvalid→validとなった場合、can_generateがFalse→Trueへ変化すること。
        検証根拠:
    ...
    def test_xy_warning_plan_can_generate(self):
        ...
        TEST ID: TEST-UNIT-087: max_x/y_deviationが非零でもgeneration_error=Falseならcan_generate=Trueであること。
        検証根拠:
        警告情報を持つplanを明示的に与えて生成可否だけを観測するため、XY警告が禁止条件へ誤昇格しないことを確認できる。
        対応要求: REQ-VALID-003, REQ-LIMIT-001
        """
    ...
    def test_rotational_error_plan_cannot_generate(self):
        ...
        TEST ID: TEST-UNIT-088: has_generation_error=Trueなら入力自体が有効でもcan_generate=Falseであること。
        検証根拠:
        Validation成功とplan生成エラーを意図的に組み合わせるため、入力エラーとは独立した回転軸禁止条件を確認できる。
        対応要求: REQ-VALID-003, REQ-LIMIT-003
        """
        self.validator.validate.return_value = Mock(is_valid=True, issues=[])
    ...
    def test_apply_settings_updates_and_revalidates(self):
        ...
        TEST ID: TEST-UNIT-089: apply_settings後、設定を保持しvalidate/build_planへ同じ設定を渡すこと。
        検証根拠:
