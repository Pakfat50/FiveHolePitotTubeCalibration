...
Details: docs/test_specification.md の TEST-UNIT-047..059 に対応する。
"""
...
class TestLimitEvaluator(unittest.TestCase):
    """軸種別ごとの可動範囲処理が要求仕様どおり異なることを確認する。"""

    def setUp(self):
        """各テストで標準可動範囲を持つ評価器を準備する。"""
        self.e = LimitEvaluator()
        self.limits = make_limits()

    def _eval(self, **kwargs):
        """指定軸だけを変更した AxisCommand を評価する補助メソッド。

        Details: 未指定軸を0に固定することで、対象軸の範囲処理を他軸から分離して観測する。
        """
    ...
    def test_all_axes_in_range(self):
        """全軸が範囲内なら飽和・回転エラーが発生しないことを確認する。

        Test: TEST-UNIT-047: 正常指令ではx/y_saturated=Falseかつrotational_error=Falseであること。
        Verification rationale:
        全軸0という明確な範囲内指令の評価結果を直接確認するため、正常系で誤警告・誤エラーがないことを判定できる。
        See Also: REQ-LIMIT-001, REQ-VALID-003
        """
        r = self._eval()
    ...
    def test_x_above_max_saturates(self):
        """X上限超過時に上限へ飽和することを確認する。

        Test: TEST-UNIT-048: X=1200に対し指令値1000、x_saturated=Trueとなること。
        Verification rationale:
        上限を200超える入力に対し、飽和後値と飽和フラグを同時確認するため、値のクランプと状態通知の両方を検証できる。
        See Also: REQ-LIMIT-001
        """
    ...
    def test_x_below_min_saturates(self):
        """X下限超過時に下限へ飽和することを確認する。

        Test: TEST-UNIT-049: X=-1200に対し指令値-1000となること。
        Verification rationale:
    ...
    def test_y_above_max_saturates(self):
        """Y上限超過時に上限へ飽和することを確認する。

        Test: TEST-UNIT-050: Y=1200に対し指令値1000となること。
        Verification rationale:
    ...
    def test_y_below_min_saturates(self):
        """Y下限超過時に下限へ飽和することを確認する。

        Test: TEST-UNIT-051: Y=-1200に対し指令値-1000となること。
        Verification rationale:
    ...
    def test_x_deviation(self):
        ...
        Test: TEST-UNIT-052: X=1200、上限1000ではx_deviation=200であること。
        Verification rationale:
    ...
    def test_y_deviation(self):
        ...
        Test: TEST-UNIT-053: Y=-1200、下限-1000ではy_deviation=200であること。
        Verification rationale:
    ...
    def test_z_above_max_not_clamped(self):
        """Z上限超過時に値を飽和せず回転エラーとすることを確認する。

        Test: TEST-UNIT-054: Z=200を保持したままrotational_error=Trueとなること。
        Verification rationale:
        出力Zが入力値のまま残ることとエラーフラグを同時確認するため、回転軸を飽和させない仕様を直接検証できる。
        See Also: REQ-LIMIT-003, REQ-VALID-003
        """
    ...
    def test_z_below_min_not_clamped(self):
        """Z下限超過時にも値を保持して回転エラーとすることを確認する。

        Test: TEST-UNIT-055: Z=-200を保持したままrotational_error=Trueとなること。
        Verification rationale:
    ...
    def test_a_above_max_not_clamped(self):
        """A上限超過時に値を飽和せず回転エラーとすることを確認する。

        Test: TEST-UNIT-056: A=800を保持したままrotational_error=Trueとなること。
        Verification rationale:
    ...
    def test_a_below_min_not_clamped(self):
        """A下限超過時に値を飽和せず回転エラーとすることを確認する。

        Test: TEST-UNIT-057: A=-800を保持したままrotational_error=Trueとなること。
        Verification rationale:
    ...
    def test_xy_only_overrange_is_not_generation_error(self):
        """X/Yだけの範囲超過では生成禁止エラーにならないことを確認する。

        Test: TEST-UNIT-058: X/Y飽和が同時発生してもrotational_error=Falseであること。
        Verification rationale:
        並進軸2軸を同時に範囲外へし、飽和フラグと回転エラーなしを確認することで、警告と生成禁止の分類を直接検証できる。
        See Also: REQ-LIMIT-001, REQ-LIMIT-003
        """
    ...
    def test_translation_and_rotation_overrange_combined(self):
        """並進軸と回転軸が同時範囲外でも各軸規則が独立適用されることを確認する。

        Test: TEST-UNIT-059: Xは1000へ飽和し、Zは200のまま保持され、回転エラーになること。
        Verification rationale:
