...
class TestGCodeGenerator(unittest.TestCase):
    ...
    def test_header_contains_required_commands(self):
        ...
        TEST ID: TEST-UNIT-066: 初期化見出し、$H、G21、G90、G94を含むこと。
        検証根拠:
    ...
    def test_initialization_text_preserved_in_order(self):
        ...
        TEST ID: TEST-UNIT-067: 複数行初期化テキストがそのまま連続部分文字列として含まれること。
        検証根拠:
        入力した複数行文字列全体の一致を確認するため、個別行の存在だけでなく行順と改行保持も検証できる。
        対応要求: REQ-INPUT-006, REQ-GCODE-002
        """
        init = ";a\nG92 X0\nM5\n"
    ...
    def test_each_move_has_four_axes_and_feed(self):
        ...
        TEST ID: TEST-UNIT-068: 全移動指令に4軸とFeed wordが存在すること。
        検証根拠:
    ...
    def test_g_numbers_are_zero_padded(self):
        ...
        TEST ID: TEST-UNIT-069: G01およびG04表記が生成されること。
        検証根拠:
    ...
    def test_feed_rate_has_six_decimal_places(self):
        ...
        TEST ID: TEST-UNIT-070: feed_rate=12.5がF12.500000となること。
        検証根拠:
        小数桁数が視認可能な値を入力し完全な期待文字列を比較するため、丸め・桁数・F接頭辞を同時に確認できる。
        対応要求: REQ-INPUT-004, REQ-GCODE-003
        """
    ...
    def test_hold_time_has_six_decimal_places(self):
        ...
        TEST ID: TEST-UNIT-071: hold_time_s=3.0がG04 P3.000000となること。
        検証根拠:
    ...
    def test_comments_enabled(self):
        ...
        TEST ID: TEST-UNIT-072: output_comments=TrueでAoA/AoSコメントを生成すること。
        検証根拠:
        オプションON時の生成文字列に較正角識別子が存在することを確認するため、コメント生成経路が有効化されることを判定できる。
        対応要求: REQ-INPUT-007, REQ-GCODE-004
        """
    ...
    def test_comments_disabled(self):
        ...
        TEST ID: TEST-UNIT-073: output_comments=FalseではAoAを含む点コメントが0件であること。
        検証根拠:
    ...
    def test_no_return_home_after_final_point(self):
        ...
        TEST ID: TEST-UNIT-074: 最終非空行が$HまたはXYZ A=0への復帰移動ではないこと。
        検証根拠:
    ...
    def test_saturated_actual_command_is_written(self):
        ...
        TEST ID: TEST-UNIT-075: saturated.command.x/yの6桁値が生成文字列に含まれること。
        ...
        対応要求: REQ-LIMIT-001, REQ-GCODE-003
        """
