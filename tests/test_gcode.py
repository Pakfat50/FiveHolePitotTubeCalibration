...
Details: docs/test_specification.md の TEST-UNIT-066..075 に対応する。
"""
...
class TestGCodeGenerator(unittest.TestCase):
    ...
    def test_header_contains_required_commands(self):
        ...
        Test: TEST-UNIT-066: 初期化見出し、$H、G21、G90、G94を含むこと。
        Verification rationale:
        仕様で列挙された必須トークンを生成文字列から直接検索するため、各必須ヘッダ要素の欠落を検出できる。
        See Also: REQ-GCODE-002
        """
        text = self._generate()
    ...
    def test_initialization_text_preserved_in_order(self):
        ...
        Test: TEST-UNIT-067: 複数行初期化テキストがそのまま連続部分文字列として含まれること。
        Verification rationale:
        入力した複数行文字列全体の一致を確認するため、個別行の存在だけでなく行順と改行保持も検証できる。
        See Also: REQ-INPUT-006, REQ-GCODE-002
        """
        init = ";a\nG92 X0\nM5\n"
    ...
    def test_each_move_has_four_axes_and_feed(self):
        ...
        Test: TEST-UNIT-068: 全移動指令に4軸とFeed wordが存在すること。
        Verification rationale:
        全G01行を抽出して各wordを反復確認するため、一部較正点だけ出力項目が欠ける不具合も検出できる。
        See Also: REQ-GCODE-003
        """
    ...
    def test_g_numbers_are_zero_padded(self):
        ...
        Test: TEST-UNIT-069: G01およびG04表記が生成されること。
        Verification rationale:
    ...
    def test_feed_rate_has_six_decimal_places(self):
        ...
        Test: TEST-UNIT-070: feed_rate=12.5がF12.500000となること。
        Verification rationale:
        小数桁数が視認可能な値を入力し完全な期待文字列を比較するため、丸め・桁数・F接頭辞を同時に確認できる。
        See Also: REQ-INPUT-004, REQ-GCODE-003
        """
    ...
    def test_hold_time_has_six_decimal_places(self):
        ...
        Test: TEST-UNIT-071: hold_time_s=3.0がG04 P3.000000となること。
        Verification rationale:
    ...
    def test_comments_enabled(self):
        ...
        Test: TEST-UNIT-072: output_comments=TrueでAoA/AoSコメントを生成すること。
        Verification rationale:
        オプションON時の生成文字列に較正角識別子が存在することを確認するため、コメント生成経路が有効化されることを判定できる。
        See Also: REQ-INPUT-007, REQ-GCODE-004
        """
    ...
    def test_comments_disabled(self):
        ...
        Test: TEST-UNIT-073: output_comments=FalseではAoAを含む点コメントが0件であること。
        Verification rationale:
    ...
    def test_no_return_home_after_final_point(self):
        ...
        Test: TEST-UNIT-074: 最終非空行が$HまたはXYZ A=0への復帰移動ではないこと。
        Verification rationale:
        終端行そのものを禁止指令集合と比較するため、生成終了時の自動復帰追加を直接検出できる。
        See Also: REQ-GCODE-005
        """
    ...
    def test_saturated_actual_command_is_written(self):
        ...
        Test: TEST-UNIT-075: saturated.command.x/yの6桁値が生成文字列に含まれること。
        Details: 実CalibrationServiceで飽和planを作り、そのPointEvaluationのactual commandを出力と比較する。
        Verification rationale:
        理想値と実値が異なる条件でactual commandを期待値に使うため、GCodeGeneratorが誤ってideal_commandを参照する不具合を検出できる。
        See Also: REQ-LIMIT-001, REQ-GCODE-003
        """
