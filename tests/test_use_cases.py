...
Details:
    docs/test_specification.md の TEST-UC-* を実装し、単体テストでは確認できないモジュール間データフローと状態遷移を確認する。
"""
...
class UseCaseHarness:
    """@brief ユースケーステストでproduction実装を組み合わせる共通ハーネス。

    Details:
        MockではなくValidator、Service、Controller、Repository、Generatorの実クラスを接続し、製品と同じ主要データ経路を再現する。
        @par 設計根拠
        ユースケーステストでは個別メソッドではなく複数モジュール間の受渡しを確認する必要があるため、共通の実オブジェクト集合を一箇所で構築する。
    """

    def __init__(self):
        """@brief ユースケース実行に必要なproductionオブジェクトを生成する。"""
...
class TestUseCases(unittest.TestCase):
    """@brief ユーザー操作単位でproductionモジュールの組み合わせ動作を確認する。"""

    def setUp(self):
        """@brief 各テストを独立状態で開始するため新しいUseCaseHarnessを生成する。"""
        self.h = UseCaseHarness()

    # TEST-UC-01-01
    # UseCase: UC-01
    def test_uc01_valid_input_recalculates_plan(self):
        """@brief 正常な較正条件入力でplanが自動生成され操作可能になることを確認する。

        Test:
            TEST-UC-01-01: 有効設定をControllerへ入力するとcurrent_planが生成されcan_generate=Trueとなること。
    ...
    def test_uc01_invalid_aoa_range_blocks_generation(self):
        """@brief AoA範囲不正で生成操作が禁止されることを確認する。

        Test:
            TEST-UC-01-02: aoa_min==aoa_maxの入力後can_generate=Falseとなること。
        Verification rationale:
    ...
    def test_uc01_temporary_invalid_then_valid_recovers(self):
        """@brief 一時的不正入力から正常入力へ戻すと生成可能状態へ復帰することを確認する。

        Test:
            TEST-UC-01-03: feed_rate=0で禁止、100へ戻した後に許可となること。
        Verification rationale:
    ...
    def test_uc01_point_count_change_rebuilds_grid(self):
        """@brief 点数変更に応じて較正点列がリアルタイム再生成されることを確認する。

        Test:
            TEST-UC-01-04: 2×2→3×4へ変更するとplan点数が4→12へ更新されること。
        Verification rationale:
    ...
    def test_uc01_serpentine_changes_order_not_set(self):
        """@brief 蛇行ON/OFFで較正点集合は同じまま走査順だけが変わることを確認する。

        Test:
            TEST-UC-01-05: 2つのplanの座標集合は一致し、順序付き列は不一致であること。
        Verification rationale:
    ...
    def test_uc01_xy_saturation_warns_but_allows_actions(self):
        """@brief X/Y飽和時に偏差が発生しても生成操作が許可されることを確認する。

        Test:
            TEST-UC-01-06: 狭いX/Y範囲でmax deviation>0かつcan_generate=Trueとなること。
        Verification rationale:
    ...
    def test_uc01_za_overrange_blocks_actions(self):
        """@brief Z/A範囲超過がplan生成禁止となり操作を無効化することを確認する。

        Test:
            TEST-UC-01-07: Z範囲±1度でhas_generation_error=Trueかつcan_generate=Falseとなること。
        Verification rationale:
    ...
    def test_uc01_xy_warning_and_za_error_coexist(self):
        """@brief X/Y警告情報を保持しつつZ/Aエラーが生成禁止として共存できることを確認する。

        Test:
            TEST-UC-01-08: X偏差>0かつhas_generation_error=Trueのplanを生成できること。
        Verification rationale:
    ...
    def test_uc01_grid_origin_is_deterministic(self):
        """@brief AoA=AoS=0の格子点がZ=A=0へ決定的に変換されることを確認する。

        Test:
            TEST-UC-01-09: 中央点command.z/aが0±0.001度であること。
        Verification rationale:
    ...
    def test_uc01_roll_has_no_unnecessary_360_jump(self):
        """@brief 蛇行走査中の隣接ロール角に不要な360度ジャンプがないことを確認する。

        Test:
            TEST-UC-01-10: 全隣接点の|ΔA|<=180.001度であること。
        Verification rationale:
    ...
    def test_uc01_nonpositive_offsets_block_plan(self):
        """@brief Lx/Lyが0以下の場合に生成操作を禁止することを確認する。

        Test:
            TEST-UC-01-11: tip_offset_xまたはtip_offset_y=0でcan_generate=Falseとなること。
        Verification rationale:
    ...
    def test_uc01_hold_and_feed_boundaries(self):
        """@brief 保持時間とFeed rateの許容下限・下限直下をユーザー入力経路で確認する。

        Test:
            TEST-UC-01-12: hold=0.1,F=1は有効で、hold<0.1またはF<1では無効となること。
        Verification rationale:
    ...
    def test_uc02_load_initialization_text(self):
        """@brief 初期化Gコードの単一行テキストをファイルから読み込めることを確認する。

        Test:
            TEST-UC-02-01: 実一時ファイル内容とRepository.load戻り値が一致すること。
        Verification rationale:
    ...
    def test_uc02_multiline_order_preserved(self):
        """@brief 複数行初期化Gコードが行順を保って読み込まれることを確認する。

        Test:
            TEST-UC-02-02: コメント・G92・M5を含む入力文字列全体が読込後に一致すること。
        Verification rationale:
    ...
    def test_uc02_cancel_keeps_current_initialization(self):
        """@brief ファイル選択キャンセルでは現在の初期化コードを変更しないことを確認する。

        Test:
            TEST-UC-02-03: selected_path=Noneのとき既存文字列が保持されること。
        Verification rationale:
    ...
    def test_uc02_load_failure_does_not_terminate_process(self):
        """@brief 初期化ファイル読込失敗を呼出し側で捕捉可能であることを確認する。

        Test:
            TEST-UC-02-04: missing fileでOSErrorが送出され、テストプロセスは継続すること。
        Verification rationale:
    ...
    def test_uc03_save_all_settings_to_csv(self):
        """@brief 全設定をCSVへ保存し同じ設定として読み戻せることを確認する。

        Test:
            TEST-UC-03-01: 標準設定のCSV round-tripが完全一致すること。
        Verification rationale:
    ...
    def test_uc03_save_option_combinations(self):
        """@brief 蛇行・コメントオプションの全組合せを保存復元できることを確認する。

        Test:
            TEST-UC-03-02: 2つのboolオプション4組合せすべてでround-tripが一致すること。
        Verification rationale:
    ...
    def test_uc03_cancel_creates_no_file(self):
        """@brief 保存キャンセル時にファイルを作成しないことを確認する。

        Test:
            TEST-UC-03-03: 操作を行わない一時ディレクトリが空のままであること。
        Verification rationale:
    ...
    def test_uc03_save_failure_is_recoverable(self):
        """@brief 設定保存I/O失敗が回復可能な例外として通知されることを確認する。

        Test:
            TEST-UC-03-04: 存在しないディレクトリへの保存でOSErrorとなること。
        Verification rationale:
    ...
    def test_uc04_load_valid_csv_and_rebuild_plan(self):
        """@brief 正常CSV読込後に設定がControllerへ適用されplanが再生成されることを確認する。

        Test:
            TEST-UC-04-01: feed_rate=123の保存読込値をapply後、current settingsへ反映されplanが存在すること。
        Verification rationale:
    ...
    def test_uc04_restore_serpentine(self):
        """@brief CSVから復元した蛇行設定が実際の走査順へ反映されることを確認する。

        Test:
            TEST-UC-04-02: serpentine=True読込後の2行目AoS順が[10,0,-10]となること。
        Verification rationale:
    ...
    def test_uc04_loaded_xy_warning_allows_generation(self):
        """@brief CSVから読み込んだX範囲が飽和警告を生じても生成可能であることを確認する。

        Test:
            TEST-UC-04-03: 狭いX範囲を保存読込・適用した後can_generate=Trueとなること。
        Verification rationale:
    ...
    def test_uc04_loaded_za_error_blocks_generation(self):
        """@brief CSVから読み込んだZ範囲が回転エラーを生じる場合に生成禁止となることを確認する。

        Test:
            TEST-UC-04-04: Z±1度設定の保存読込・適用後can_generate=Falseとなること。
        Verification rationale:
    ...
    def test_uc04_structurally_invalid_csv_is_rejected(self):
        """@brief 構造不正CSVが設定として受理されないことを確認する。

        Test:
            TEST-UC-04-05: key/value構造でないCSVがSettingsLoadErrorとなること。
        Verification rationale:
    ...
    def test_uc04_cancel_keeps_settings_and_plan(self):
        """@brief 設定読込キャンセルで現在設定とplanが保持されることを確認する。

        Test:
            TEST-UC-04-06: selected=Noneのまま既存feed_rate=55とplanオブジェクトが変化しないこと。
        Verification rationale:
    ...
    def test_uc04_missing_required_value_is_rejected(self):
        """@brief 必須項目が欠損したCSVを拒否することを確認する。

        Test:
            TEST-UC-04-07: 一部キーしか持たないCSVがSettingsLoadErrorとなること。
        Verification rationale:
    ...
    def test_uc04_blank_required_value_is_rejected(self):
        """@brief 必須値が空欄のCSVを拒否することを確認する。

        Test:
            TEST-UC-04-08: 正常CSVのfeed_rateだけ空欄にするとSettingsLoadErrorとなること。
        Verification rationale:
    ...
    def test_uc04_non_numeric_value_is_rejected(self):
        """@brief 数値項目に非数値文字列を持つCSVを拒否することを確認する。

        Test:
            TEST-UC-04-09: feed_rate=abcでSettingsLoadErrorとなること。
        Verification rationale:
    ...
    def test_uc04_io_failure_is_rejected(self):
        """@brief 設定CSVのI/O失敗をSettingsLoadErrorとして扱うことを確認する。

        Test:
            TEST-UC-04-10: 存在しないCSVパスのloadがSettingsLoadErrorとなること。
        Verification rationale:
    ...
    def test_uc04_late_error_does_not_partially_apply(self):
        """@brief CSV後半項目のエラーでも前半の値を部分適用しないことを確認する。

        Test:
            TEST-UC-04-11: output_comments不正でload失敗した後、既存current settingsが完全一致で保持されること。
        Verification rationale:
    ...
    def test_uc05_normal_simulation_uses_full_plan(self):
        """@brief 正常planのシミュレーションが最終較正点まで走査できることを確認する。

        Test:
            TEST-UC-05-01: start後progress=1.0がplan最終点を返すこと。
        Verification rationale:
    ...
    def test_uc05_xy_warning_plan_is_simulatable(self):
        """@brief X/Y飽和を含むplanでもシミュレーション禁止エラーにならないことを確認する。

        Test:
            TEST-UC-05-02: X範囲を狭めたplanのhas_generation_error=Falseであること。
        Verification rationale:
    ...
    def test_uc05_za_error_is_not_simulatable_from_gui_state(self):
        """@brief Z/A範囲エラー時にGUI相当の生成可否がFalseとなることを確認する。

        Test:
            TEST-UC-05-03: Z±1度設定後Controller.can_generate=Falseであること。
        Verification rationale:
    ...
    def test_uc05_duration_does_not_depend_on_hold_time(self):
        """@brief 較正点保持時間が異なってもシミュレーション全体時間が同じことを確認する。

        Test:
            TEST-UC-05-04: hold=0.1と100のplanを双方duration=10で開始しduration_sが一致すること。
        Verification rationale:
    ...
    def test_uc05_display_information_matches_plan(self):
        """@brief シミュレーション状態表示が描画対象plan点の情報を使用することを確認する。

        Test:
            TEST-UC-05-05: render_frameしたpointのindexがstatus_textへ含まれること。
        Verification rationale:
    ...
    def test_uc05_two_views_share_same_current_point(self):
        """@brief 機構2面図の現在点状態が同一PointEvaluationに基づくことを確認する。

        Test:
            TEST-UC-05-06: plan.points[1]描画後current_point_indexが同点indexとなること。
        Verification rationale:
    ...
    def test_uc05_simulation_displays_all_calibration_points_without_legend(self):
        """@brief シミュレーション画面に全較正点マップが凡例なしで表示されることを確認する。

        Test:
            TEST-UC-05-07: 3×4planの全点数とscatter offset数が一致し、AoS/AoA軸ラベルを持ちlegend=Noneであること。
        Verification rationale:
    ...
    def test_uc05_three_views_share_same_current_point(self):
        """@brief 横面図・正面図・較正点マップの現在点がフレーム更新ごとに同期することを確認する。

        Test:
            TEST-UC-05-08: 異なる2フレームでcurrent indexとmap強調座標が同一pointのAoS/AoAへ追従し、文字注記・凡例を追加しないこと。
        Verification rationale:
    ...
    def test_uc06_generate_valid_nc(self):
        """@brief 正常planから必須ヘッダと全較正点移動を含むGコードを生成できることを確認する。

        Test:
            TEST-UC-06-01: 2×2planの出力に$Hを含みG01行が4件であること。
        Verification rationale:
    ...
    def test_uc06_comments_on(self):
        """@brief コメントON設定が生成Gコードの較正点コメントへ反映されることを確認する。

        Test:
            TEST-UC-06-02: output_comments=Trueの出力にAoAが含まれること。
        Verification rationale:
    ...
    def test_uc06_comments_off(self):
        """@brief コメントOFF設定で較正点コメントを出力しないことを確認する。

        Test:
            TEST-UC-06-03: output_comments=Falseで;始まりのAoAコメントが存在しないこと。
        Verification rationale:
    ...
    def test_uc06_xy_saturated_values_are_written(self):
        """@brief X飽和を含むplanでは飽和後のactual commandをGコードへ書くことを確認する。

        Test:
            TEST-UC-06-04: x_saturated点のcommand.x値が6桁形式で出力文字列に含まれること。
        Verification rationale:
    ...
    def test_uc06_za_error_blocks_generation_action(self):
        """@brief Z/A範囲エラー時にGコード生成操作が許可されないことを確認する。

        Test:
            TEST-UC-06-05: Z±1度設定後Controller.can_generate=Falseであること。
        Verification rationale:
    ...
    def test_uc06_feed_and_hold_are_written(self):
        """@brief 設定したFeed rateと保持時間がGコードへ規定書式で反映されることを確認する。

        Test:
            TEST-UC-06-06: F12.500000およびG04 P3.000000を含むこと。
        Verification rationale:
    ...
    def test_uc06_loaded_initialization_is_in_header(self):
        """@brief 読み込んだ初期化Gコードが生成Gコードヘッダへ順序保持で含まれることを確認する。
    ...
    def test_uc06_final_point_has_no_return_home(self):
        """@brief Gコード終了後に原点復帰指令を追加しないことを確認する。

        Test:
            TEST-UC-06-08: 最終非空行が$Hまたは原点へのG00/G01ではないこと。
        Verification rationale:
    ...
    def test_uc06_cancel_creates_no_file(self):
        """@brief Gコード保存キャンセル時にファイル副作用がないことを確認する。

        Test:
            TEST-UC-06-09: 保存処理を行わない一時ディレクトリが空のままであること。
        Verification rationale:
    ...
    def test_uc06_save_failure_is_recoverable(self):
        """@brief Gコード保存失敗が回復可能なI/O例外として通知されることを確認する。

        Test:
            TEST-UC-06-10: 存在しないディレクトリへのsaveでOSErrorとなること。
        Verification rationale:
    ...
    def test_uc06_display_simulation_and_gcode_use_same_commands(self):
        """@brief CalibrationPlanの各actual commandがそのままGコードへ出力されることを確認する。

        Test:
            TEST-UC-06-11: 全plan点のX/Y/Z/A command値が6桁形式で生成文字列に存在すること。
        Verification rationale:
    ...
    def test_uc06_all_float_words_have_six_decimals(self):
        """@brief 生成Gコードの全浮動小数点wordが小数点以下6桁であることを確認する。

        Test:
            TEST-UC-06-12: G01/G04のXYZAFP wordすべてが正規表現^-?digits.6digits$形式を満たすこと。
        Verification rationale:
    ...
    def _with_text_file(self,text,callback):
        """@brief 指定テキストの一時ファイルを作成しcallback実行後に必ず削除する。
    ...
    def _round_trip_settings(self,settings):
        """@brief 設定を一時CSVへ保存・再読込し完全一致を確認する。
    ...
    def _assert_csv_rejected(self,text):
        """@brief 指定CSV文字列がSettingsLoadErrorとして拒否されることを確認する。"""
        with self.assertRaises(SettingsLoadError): self._load_csv_text(text)

    def _load_csv_text(self,text):
        """@brief CSV文字列を実一時ファイル経由でSettingsRepositoryへ入力する。
    ...
    def _valid_csv(self):
        """@brief 異常CSV試験の基準となる全必須項目入り正常CSV文字列を返す。
