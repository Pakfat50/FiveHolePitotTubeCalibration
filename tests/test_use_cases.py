...
class TestUseCases(unittest.TestCase):
    ...
    def test_uc01_valid_input_recalculates_plan(self):
        ...
        TEST ID: TEST-UC-01-01: 有効設定をControllerへ入力するとcurrent_planが生成されcan_generate=Trueとなること。
        検証根拠:
    ...
    def test_uc01_invalid_aoa_range_blocks_generation(self):
        ...
        TEST ID: TEST-UC-01-02: aoa_min==aoa_maxの入力後can_generate=Falseとなること。
        検証根拠:
    ...
    def test_uc01_temporary_invalid_then_valid_recovers(self):
        ...
        TEST ID: TEST-UC-01-03: feed_rate=0で禁止、100へ戻した後に許可となること。
        検証根拠:
    ...
    def test_uc01_point_count_change_rebuilds_grid(self):
        ...
        TEST ID: TEST-UC-01-04: 2×2→3×4へ変更するとplan点数が4→12へ更新されること。
        検証根拠:
    ...
    def test_uc01_serpentine_changes_order_not_set(self):
        ...
        TEST ID: TEST-UC-01-05: 2つのplanの座標集合は一致し、順序付き列は不一致であること。
        検証根拠:
    ...
    def test_uc01_xy_saturation_warns_but_allows_actions(self):
        ...
        TEST ID: TEST-UC-01-06: 狭いX/Y範囲でmax deviation>0かつcan_generate=Trueとなること。
        検証根拠:
    ...
    def test_uc01_za_overrange_blocks_actions(self):
        ...
        TEST ID: TEST-UC-01-07: Z範囲±1度でhas_generation_error=Trueかつcan_generate=Falseとなること。
        検証根拠:
    ...
    def test_uc01_xy_warning_and_za_error_coexist(self):
        ...
        TEST ID: TEST-UC-01-08: X偏差>0かつhas_generation_error=Trueのplanを生成できること。
        検証根拠:
    ...
    def test_uc01_grid_origin_is_deterministic(self):
        ...
        TEST ID: TEST-UC-01-09: 中央点command.z/aが0±0.001度であること。
        検証根拠:
    ...
    def test_uc01_roll_has_no_unnecessary_360_jump(self):
        ...
        TEST ID: TEST-UC-01-10: 全隣接点の|ΔA|<=180.001度であること。
        検証根拠:
    ...
    def test_uc01_nonpositive_offsets_block_plan(self):
        ...
        TEST ID: TEST-UC-01-11: tip_offset_xまたはtip_offset_y=0でcan_generate=Falseとなること。
        検証根拠:
    ...
    def test_uc01_hold_and_feed_boundaries(self):
        ...
        TEST ID: TEST-UC-01-12: hold=0.1,F=1は有効で、hold<0.1またはF<1では無効となること。
        検証根拠:
    ...
    def test_uc02_load_initialization_text(self):
        ...
        TEST ID: TEST-UC-02-01: 実一時ファイル内容とRepository.load戻り値が一致すること。
        検証根拠:
    ...
    def test_uc02_multiline_order_preserved(self):
        ...
        TEST ID: TEST-UC-02-02: コメント・G92・M5を含む入力文字列全体が読込後に一致すること。
        検証根拠:
    ...
    def test_uc02_cancel_keeps_current_initialization(self):
        ...
        TEST ID: TEST-UC-02-03: selected_path=Noneのとき既存文字列が保持されること。
        検証根拠:
    ...
    def test_uc02_load_failure_does_not_terminate_process(self):
        ...
        TEST ID: TEST-UC-02-04: missing fileでOSErrorが送出され、テストプロセスは継続すること。
        検証根拠:
    ...
    def test_uc03_save_all_settings_to_csv(self):
        ...
        TEST ID: TEST-UC-03-01: 標準設定のCSV round-tripが完全一致すること。
        検証根拠:
    ...
    def test_uc03_save_option_combinations(self):
        ...
        TEST ID: TEST-UC-03-02: 2つのboolオプション4組合せすべてでround-tripが一致すること。
        検証根拠:
    ...
    def test_uc03_cancel_creates_no_file(self):
        ...
        TEST ID: TEST-UC-03-03: 操作を行わない一時ディレクトリが空のままであること。
        検証根拠:
    ...
    def test_uc03_save_failure_is_recoverable(self):
        ...
        TEST ID: TEST-UC-03-04: 存在しないディレクトリへの保存でOSErrorとなること。
        検証根拠:
    ...
    def test_uc04_load_valid_csv_and_rebuild_plan(self):
        ...
        TEST ID: TEST-UC-04-01: feed_rate=123の保存読込値をapply後、current settingsへ反映されplanが存在すること。
        検証根拠:
    ...
    def test_uc04_restore_serpentine(self):
        ...
        TEST ID: TEST-UC-04-02: serpentine=True読込後の2行目AoS順が[10,0,-10]となること。
        検証根拠:
    ...
    def test_uc04_loaded_xy_warning_allows_generation(self):
        ...
        TEST ID: TEST-UC-04-03: 狭いX範囲を保存読込・適用した後can_generate=Trueとなること。
        検証根拠:
    ...
    def test_uc04_loaded_za_error_blocks_generation(self):
        ...
        TEST ID: TEST-UC-04-04: Z±1度設定の保存読込・適用後can_generate=Falseとなること。
        検証根拠:
    ...
    def test_uc04_structurally_invalid_csv_is_rejected(self):
        ...
        TEST ID: TEST-UC-04-05: key/value構造でないCSVがSettingsLoadErrorとなること。
        検証根拠:
    ...
    def test_uc04_cancel_keeps_settings_and_plan(self):
        ...
        TEST ID: TEST-UC-04-06: selected=Noneのまま既存feed_rate=55とplanオブジェクトが変化しないこと。
        検証根拠:
    ...
    def test_uc04_missing_required_value_is_rejected(self):
        ...
        TEST ID: TEST-UC-04-07: 一部キーしか持たないCSVがSettingsLoadErrorとなること。
        検証根拠:
    ...
    def test_uc04_blank_required_value_is_rejected(self):
        ...
        TEST ID: TEST-UC-04-08: 正常CSVのfeed_rateだけ空欄にするとSettingsLoadErrorとなること。
        検証根拠:
    ...
    def test_uc04_non_numeric_value_is_rejected(self):
        ...
        TEST ID: TEST-UC-04-09: feed_rate=abcでSettingsLoadErrorとなること。
        検証根拠:
    ...
    def test_uc04_io_failure_is_rejected(self):
        ...
        TEST ID: TEST-UC-04-10: 存在しないCSVパスのloadがSettingsLoadErrorとなること。
        検証根拠:
    ...
    def test_uc04_late_error_does_not_partially_apply(self):
        ...
        TEST ID: TEST-UC-04-11: output_comments不正でload失敗した後、既存current settingsが完全一致で保持されること。
        検証根拠:
    ...
    def test_uc05_normal_simulation_uses_full_plan(self):
        ...
        TEST ID: TEST-UC-05-01: start後progress=1.0がplan最終点を返すこと。
        検証根拠:
    ...
    def test_uc05_xy_warning_plan_is_simulatable(self):
        ...
        TEST ID: TEST-UC-05-02: X範囲を狭めたplanのhas_generation_error=Falseであること。
        検証根拠:
    ...
    def test_uc05_za_error_is_not_simulatable_from_gui_state(self):
        ...
        TEST ID: TEST-UC-05-03: Z±1度設定後Controller.can_generate=Falseであること。
        検証根拠:
    ...
    def test_uc05_duration_does_not_depend_on_hold_time(self):
        ...
        TEST ID: TEST-UC-05-04: hold=0.1と100のplanを双方duration=10で開始しduration_sが一致すること。
        検証根拠:
    ...
    def test_uc05_display_information_matches_plan(self):
        ...
        TEST ID: TEST-UC-05-05: render_frameしたpointのindexがstatus_textへ含まれること。
        検証根拠:
    ...
    def test_uc05_two_views_share_same_current_point(self):
        ...
        TEST ID: TEST-UC-05-06: plan.points[1]描画後current_point_indexが同点indexとなること。
        検証根拠:
    ...
    def test_uc05_simulation_displays_all_calibration_points_without_legend(self):
        ...
        TEST ID: TEST-UC-05-07: 3×4planの全点数とscatter offset数が一致し、AoS/AoA軸ラベルを持ちlegend=Noneであること。
        検証根拠:
    ...
    def test_uc05_three_views_share_same_current_point(self):
        ...
        TEST ID: TEST-UC-05-08: 異なる2フレームでcurrent indexとmap強調座標が同一pointのAoS/AoAへ追従し、文字注記・凡例を追加しないこと。
        検証根拠:
    ...
    def test_uc06_generate_valid_nc(self):
        ...
        TEST ID: TEST-UC-06-01: 2×2planの出力に$Hを含みG01行が4件であること。
        検証根拠:
    ...
    def test_uc06_comments_on(self):
        ...
        TEST ID: TEST-UC-06-02: output_comments=Trueの出力にAoAが含まれること。
        検証根拠:
    ...
    def test_uc06_comments_off(self):
        ...
        TEST ID: TEST-UC-06-03: output_comments=Falseで;始まりのAoAコメントが存在しないこと。
        検証根拠:
    ...
    def test_uc06_xy_saturated_values_are_written(self):
        ...
        TEST ID: TEST-UC-06-04: x_saturated点のcommand.x値が6桁形式で出力文字列に含まれること。
        検証根拠:
    ...
    def test_uc06_za_error_blocks_generation_action(self):
        ...
        TEST ID: TEST-UC-06-05: Z±1度設定後Controller.can_generate=Falseであること。
        検証根拠:
    ...
    def test_uc06_feed_and_hold_are_written(self):
        ...
        TEST ID: TEST-UC-06-06: F12.500000およびG04 P3.000000を含むこと。
        検証根拠:
    ...
    def test_uc06_final_point_has_no_return_home(self):
        ...
        TEST ID: TEST-UC-06-08: 最終非空行が$Hまたは原点へのG00/G01ではないこと。
        検証根拠:
    ...
    def test_uc06_cancel_creates_no_file(self):
        ...
        TEST ID: TEST-UC-06-09: 保存処理を行わない一時ディレクトリが空のままであること。
        検証根拠:
    ...
    def test_uc06_save_failure_is_recoverable(self):
        ...
        TEST ID: TEST-UC-06-10: 存在しないディレクトリへのsaveでOSErrorとなること。
        検証根拠:
    ...
    def test_uc06_display_simulation_and_gcode_use_same_commands(self):
        ...
        TEST ID: TEST-UC-06-11: 全plan点のX/Y/Z/A command値が6桁形式で生成文字列に存在すること。
        検証根拠:
    ...
    def test_uc06_all_float_words_have_six_decimals(self):
        ...
        TEST ID: TEST-UC-06-12: G01/G04のXYZAFP wordすべてが正規表現^-?digits.6digits$形式を満たすこと。
        検証根拠:
