...
Details: docs/test_specification.md の TEST-UNIT-017..024 に対応する。
"""
...
class TestScanPlanner(unittest.TestCase):
    ...
    def test_minimum_grid_has_four_endpoints(self):
        ...
        Test: TEST-UNIT-017: AoA/AoS各2点では両端の直積4点を生成すること。
        Details: 生成点数とAoA/AoS座標集合を同時に比較する。
        Verification rationale:
        点数だけでなく4つの端点座標そのものを照合するため、両端を含む格子生成を直接確認できる。
        See Also: REQ-SCAN-001
        """
    ...
    def test_aoa_equal_spacing(self):
        ...
        Test: TEST-UNIT-018: -10～10度を5点指定した場合、AoAが[-10,-5,0,5,10]となること。
        Details: 生成点からAoAの一意値を抽出し、理論上の等間隔列と比較する。
        Verification rationale:
    ...
    def test_aos_equal_spacing(self):
        ...
        Test: TEST-UNIT-019: -20～20度を5点指定した場合、AoSが[-20,-10,0,10,20]となること。
        Details: 生成点からAoSの一意値を抽出し、期待する等間隔列と比較する。
        Verification rationale:
    ...
    def test_basic_scan_aoa_outer_aos_inner(self):
        ...
        Test: TEST-UNIT-020: serpentine=Falseでは同一AoA行内でAoSを走査し、完了後に次AoAへ移ること。
        Details: 2×3格子の生成順を期待する6点列と完全一致比較する。
        Verification rationale:
        集合ではなく順序付き座標列を比較するため、ループの入れ子順序まで確認できる。
        See Also: REQ-SCAN-002
        """
    ...
    def test_serpentine_second_row_reversed(self):
        ...
        Test: TEST-UNIT-021: serpentine=Trueでは隣接AoA行ごとにAoS走査方向を反転すること。
        Details: 2×3格子の全走査順を期待列と比較する。
        Verification rationale:
        1行目の昇順と2行目の降順を同時に観測するため、蛇行反転動作を直接確認できる。
        See Also: REQ-SCAN-003
        """
    ...
    def test_serpentine_three_rows_alternate(self):
        ...
        Test: TEST-UNIT-022: 3行のAoS方向が昇順→降順→昇順となること。
        Details: 3×3格子をAoA行ごとに分割し、各行のAoS配列を比較する。
        Verification rationale:
    ...
    def test_total_point_count(self):
        ...
        Test: TEST-UNIT-023: 4×5指定では20点を生成すること。
        Details: 生成リスト長を点数の直積と比較する。
        Verification rationale:
    ...
    def test_indices_are_unique_and_sequential(self):
        ...
        Test: TEST-UNIT-024: 全点のindexが重複なく0..N-1であること。
        Details: 生成順のindex列をrange(N)と完全一致比較する。
        Verification rationale:
        順序付きindex列を直接比較するため、重複、欠番、走査順との不一致を同時に検出できる。
        See Also: REQ-SCAN-001, REQ-SCAN-002
        """
