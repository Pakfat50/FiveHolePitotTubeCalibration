...
class TestScanPlanner(unittest.TestCase):
    ...
    def test_minimum_grid_has_four_endpoints(self):
        ...
        TEST ID: TEST-UNIT-017: AoA/AoS各2点では両端の直積4点を生成すること。
    ...
    def test_aoa_equal_spacing(self):
        ...
        TEST ID: TEST-UNIT-018: -10～10度を5点指定した場合、AoAが[-10,-5,0,5,10]となること。
    ...
    def test_aos_equal_spacing(self):
        ...
        TEST ID: TEST-UNIT-019: -20～20度を5点指定した場合、AoSが[-20,-10,0,10,20]となること。
    ...
    def test_basic_scan_aoa_outer_aos_inner(self):
        ...
        TEST ID: TEST-UNIT-020: serpentine=Falseでは同一AoA行内でAoSを走査し、完了後に次AoAへ移ること。
    ...
    def test_serpentine_second_row_reversed(self):
        ...
        TEST ID: TEST-UNIT-021: serpentine=Trueでは隣接AoA行ごとにAoS走査方向を反転すること。
        2×3格子の全走査順を期待列と比較する。
        検証根拠:
    ...
    def test_serpentine_three_rows_alternate(self):
        ...
        TEST ID: TEST-UNIT-022: 3行のAoS方向が昇順→降順→昇順となること。
    ...
    def test_total_point_count(self):
        ...
        TEST ID: TEST-UNIT-023: 4×5指定では20点を生成すること。
        生成リスト長を点数の直積と比較する。
        検証根拠:
    ...
    def test_indices_are_unique_and_sequential(self):
        ...
        TEST ID: TEST-UNIT-024: 全点のindexが重複なく0..N-1であること。
        生成順のindex列をrange(N)と完全一致比較する。
        検証根拠:
        順序付きindex列を直接比較するため、重複、欠番、走査順との不一致を同時に検出できる。
        対応要求: REQ-SCAN-001, REQ-SCAN-002
        """
