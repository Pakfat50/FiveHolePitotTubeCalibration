...
class TestMainWindow(unittest.TestCase):
    """build_ui=FalseでMainWindowのイベント制御と表示状態をGUI実体から分離して確認する。"""

    def setUp(self):
        """MainWindowの全外部依存をMock化して制御ロジックだけを観測可能にする。"""
    ...
    def test_required_japanese_labels_and_buttons_are_defined(self):
        ...
        パスクライテリア:
            シミュレーション、Gコード生成、設定保存、設定読込の4ラベルを持つこと。
    ...
    def test_validation_error_changes_target_background_and_shows_existing_message(self):
        ...
        テスト手順:
            1. エラー用背景色はレビュー承認済みの薄い赤 #FFECEC とする。エラーstyleはfieldbackgroundだけを設定し、枠設定や新規GUI要素を追加しない。
            2. テスト対象のメソッドを呼び出して結果を取得する。
            3. 取得した結果を期待値と比較する。

        パスクライテリア:
            feed_rateエラー時、feed_rateだけをエラーstyleとし、status_messageへ理由を表示し、モーダル表示しないこと。
    ...
    def test_validation_error_background_and_message_clear_after_recovery(self):
        ...
        パスクライテリア:
            issue有り→issue無しの連続更新後、feed_rateへ通常styleが再適用され、入力エラー理由が消えること。
    ...
    def test_xy_warning_shows_separate_deviations_without_resultant(self):
        ...
        パスクライテリア:
            status_messageにX=1.25,Y=2.5を含み「合成」を含まないこと。
    ...
    def test_rotational_error_disables_actions(self):
        ...
        パスクライテリア:
            can_generate=Falseでsimulation_enabled=Falseかつgcode_enabled=Falseとなること。
    ...
    def test_valid_plan_enables_actions(self):
        ...
        パスクライテリア:
            can_generate=Trueでsimulation_enabled/gcode_enabled=Trueとなること。
    ...
    def test_initialization_gcode_load_success(self):
        ...
        テスト手順:
            1. "がinitialization_textへ保存されること。
            2. テスト対象のメソッドを呼び出して結果を取得する。
            3. 取得した結果を期待値と比較する。

        パスクライテリア:
            Repository戻り値"G92 X0。
    ...
    def test_save_settings_passes_current_settings_to_repository(self):
        ...
        パスクライテリア:
            _on_save_settings("settings.csv")でsave(path,current_settings)を1回呼ぶこと。
    ...
    def test_load_settings_applies_and_revalidates(self):
        ...
        パスクライテリア:
            Repositoryのloaded settingsをapply_settingsへ1回渡すこと。
    ...
    def test_simulation_uses_current_plan(self):
        ...
        パスクライテリア:
            SimulationController.start(plan,duration_s=10.0)を1回呼ぶこと。
    ...
    def test_generate_gcode_sequence(self):
        ...
        パスクライテリア:
            Generatorへplan/settings/initを渡し、生成文字列を指定pathへsaveすること。
    ...
    def test_failed_csv_load_keeps_existing_state_and_notifies_user(self):
        ...
        テスト手順:
            1. 既存settings/planを保持した状態でRepositoryだけを失敗させる。
            2. テスト対象のメソッドを呼び出して結果を取得する。
            3. 取得した結果を期待値と比較する。

        パスクライテリア:
            SettingsLoadError時にapply_settingsを呼ばず、理由をstatus_messageへ表示し、モーダル要求しないこと。
    ...
    def _prepare_generation_state(self, can_generate: bool) -> None:
        """操作イベントテスト用にControllerの生成可否を固定する。"""
