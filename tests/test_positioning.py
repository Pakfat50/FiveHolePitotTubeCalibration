...
class TestPositionCompensator(unittest.TestCase):
    ...
    def test_zero_pitch_requires_no_translation(self):
        ...
        テスト手順:
            1. Lx=100,Ly=10の非零寸法を用い、角度だけを0度にして補正量を観測する。
            2. テスト対象のメソッドを呼び出して結果を取得する。
            3. 取得した結果を期待値と比較する。

        パスクライテリア:
            theta=0度ではX=0,Y=0となること。
    ...
    def test_positive_pitch_matches_formula(self):
        ...
        テスト手順:
            1. 独立に計算した理論値と実装出力を許容誤差内で比較する。
            2. テスト対象のメソッドを呼び出して結果を取得する。
            3. 取得した結果を期待値と比較する。

        パスクライテリア:
            theta=+15度の計算結果がREQ-POS-001の式に一致すること。
    ...
    def test_negative_pitch_matches_formula(self):
        ...
        テスト手順:
            1. 負角度を使用してsin項の符号反転を含む結果を比較する。
            2. テスト対象のメソッドを呼び出して結果を取得する。
            3. 取得した結果を期待値と比較する。

        パスクライテリア:
            theta=-15度の計算結果が補正式に一致すること。
    ...
    def test_small_positive_ly(self):
        ...
        テスト手順:
            1. Ly=1e-6を使用し、退化形状近傍での出力を比較する。
            2. テスト対象のメソッドを呼び出して結果を取得する。
            3. 取得した結果を期待値と比較する。

        パスクライテリア:
            Ly→0+近傍でも数値計算が理論式に一致すること。
    ...
    def test_small_positive_lx(self):
        ...
        テスト手順:
            1. Lx=1e-6を使用してLy支配の条件を検証する。
            2. テスト対象のメソッドを呼び出して結果を取得する。
            3. 取得した結果を期待値と比較する。

        パスクライテリア:
            Lx→0+近傍でも数値計算が理論式に一致すること。
    ...
    def test_roll_does_not_affect_xy(self):
        ...
        テスト手順:
            1. 同一のtheta/Lx/Lyを2回計算し結果が完全一致することを確認する。
            2. テスト対象のメソッドを呼び出して結果を取得する。
            3. 取得した結果を期待値と比較する。

        パスクライテリア:
            PositionCompensatorの計算入力はtheta/Lx/Lyのみで、同一入力から常に同一XYを得ること。
