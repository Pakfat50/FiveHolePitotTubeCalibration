...
class MainWindow:
    ...
    DEFAULT_VALUES = {
        "aoa_min": "-10.0", "aoa_max": "10.0", "aos_min": "-10.0", "aos_max": "10.0",
        "aoa_points": "5", "aos_points": "5", "tip_offset_x": "100.0", "tip_offset_y": "10.0",
        "hold_time_s": "1.0", "feed_rate": "100.0", "x_min": "-1000.0", "x_max": "1000.0",
    ...
    def _build_input_panel(self, parent) -> None:
        ...
        # 短い操作名として、狭い左ペインでもボタン文字列が見切れないようにする。
        ttk.Button(initialization, text="読込", command=self._choose_initialization_file, width=8).pack(side="right")
    ...
    def _settings_from_widgets(self) -> CalibrationSettings:
        ...
        def f(key: str) -> float: return float(self._widget_vars[key].get())
        def i(key: str) -> int: return int(self._widget_vars[key].get())
        return CalibrationSettings(
            aoa_min=f("aoa_min"), aoa_max=f("aoa_max"), aos_min=f("aos_min"), aos_max=f("aos_max"),
            aoa_points=i("aoa_points"), aos_points=i("aos_points"), tip_offset_x=f("tip_offset_x"),
            tip_offset_y=f("tip_offset_y"), hold_time_s=f("hold_time_s"), feed_rate=f("feed_rate"),
            axis_limits=AxisLimits(x=AxisRange(f("x_min"), f("x_max")), y=AxisRange(f("y_min"), f("y_max")),
                                   z=AxisRange(f("z_min"), f("z_max")), a=AxisRange(f("a_min"), f("a_max"))),
    ...
    def _on_input_changed(self, raw_input=None) -> None:
        ...
        if raw_input is None: raw_input = self._collect_raw_input()
        ...
        if isinstance(plan, CalibrationPlan):
            self.map_view.render(plan); self.map_view.axes.grid(True, alpha=0.3); self._update_plan_status(plan)
    ...
    def _update_plan_status(self, plan) -> None:
        ...
        if plan.has_generation_error: messages.append("Z/A可動範囲外のため生成できません。")
        if messages: self.status_message = " / ".join(messages)
        elif not self.field_errors: self.status_message = f"入力値は有効です。較正点数: {len(plan.points)}"
    ...
    def _update_action_state(self) -> None:
        ...
        allowed = bool(self.controller.can_generate()); self.simulation_enabled = allowed; self.gcode_enabled = allowed
        self._update_button_widgets()

    def _update_button_widgets(self) -> None:
        """保持中の操作可否を実際のTkボタンへ反映する。"""
        if hasattr(self, "simulation_button"): self.simulation_button.configure(state="normal" if self.simulation_enabled else "disabled")
        if hasattr(self, "gcode_button"): self.gcode_button.configure(state="normal" if self.gcode_enabled else "disabled")

    # 対応要求: REQ-INPUT-006, REQ-GUI-005
    def _on_load_initialization(self, path: str | None = None) -> None:
        """初期化Gコードを読み込み、失敗時は現在状態を維持する。"""
        if path is None: return
        try: text = self.initialization_repository.load(path)
        except (OSError, UnicodeError) as exc:
            self.status_message = f"初期化Gコードを読み込めません: {exc}"; self.modal_dialog_requested = False; self._refresh_status_widget(); return
        self.initialization_text = text
        if hasattr(self, "initialization_path_label"): self.initialization_path_label.configure(text=path)
        self.status_message = "初期化Gコードを読み込みました。"; self._refresh_status_widget()
    ...
    def _choose_initialization_file(self) -> None:
        ...
        if path: self._on_load_initialization(path)
    ...
    def _on_save_settings(self, path: str | None = None) -> None:
        ...
        try: self.settings_repository.save(path, settings)
        except OSError as exc:
            self.status_message = f"設定を保存できません: {exc}"; self.modal_dialog_requested = False; self._refresh_status_widget(); return
        self.status_message = "設定を保存しました。"; self._refresh_status_widget()

    def _choose_save_settings(self) -> None:
        """設定CSV保存先を選択する。"""
        path = filedialog.asksaveasfilename(title="設定を保存", defaultextension=".csv", filetypes=[("CSV", "*.csv")])
        if path: self._on_save_settings(path)

    # 対応要求: REQ-GUI-003, REQ-GUI-004, REQ-GUI-005
    def _on_load_settings(self, path: str | None = None) -> None:
        """CSV設定を一括で読み込み、失敗時は現在状態を維持する。"""
        if path is None: return
        try: settings = self.settings_repository.load(path)
        except SettingsLoadError as exc:
            self.status_message = str(exc); self.modal_dialog_requested = False; self._refresh_status_widget(); return
        validation = self.controller.apply_settings(settings)
        if isinstance(validation, ValidationResult): self._update_validation_display(validation)
    ...
    def _choose_load_settings(self) -> None:
        ...
        if path: self._on_load_settings(path)

    def _apply_settings_to_widgets(self, settings: CalibrationSettings) -> None:
        """読み込んだ設定を入力欄へ一括反映する。"""
        if not self._widget_vars: return
        values = {"aoa_min": settings.aoa_min, "aoa_max": settings.aoa_max, "aos_min": settings.aos_min,
                  "aos_max": settings.aos_max, "aoa_points": settings.aoa_points, "aos_points": settings.aos_points,
        ...
        try:
            for key, value in values.items(): self._widget_vars[key].set(str(value))
            self.serpentine_var.set(settings.serpentine); self.output_comments_var.set(settings.output_comments)
        finally: self._updating_widgets = False

    # 対応要求: REQ-SIM-001, REQ-GUI-004
    def _on_simulate(self) -> None:
        """現在の較正計画を使用して約10秒のシミュレーションを開始する。"""
        plan = self.controller.get_current_plan()
        if plan is None or not self.controller.can_generate(): return
        self.simulation_controller.start(plan, duration_s=10.0)
        view = getattr(self.simulation_controller, "view", None); figure = getattr(view, "figure", None)
        if figure is not None: figure.show()

    # 対応要求: REQ-GCODE-001, REQ-GUI-004
    def _on_generate_gcode(self, path: str | None = None) -> None:
        """現在の共有較正計画から`.nc`文字列を生成し保存する。"""
        if path is None: return
        plan = self.controller.get_current_plan(); settings = self.controller.get_current_settings()
        if plan is None or settings is None or not self.controller.can_generate(): return
        text = self.gcode_generator.generate(plan, settings, self.initialization_text)
        try: self.gcode_repository.save(path, text)
        except OSError as exc:
            self.status_message = f"Gコードを保存できません: {exc}"; self.modal_dialog_requested = False; self._refresh_status_widget(); return
        self.status_message = "Gコードを保存しました。"; self._refresh_status_widget()
    ...
    def _choose_generate_gcode(self) -> None:
        ...
        if path: self._on_generate_gcode(path)

    def _refresh_status_widget(self) -> None:
        """ヘッドレステストでは何もせず、実GUIがある場合だけ状態ラベルを更新する。"""
        if hasattr(self, "status_label"): self.status_label.configure(text=self.status_message)
